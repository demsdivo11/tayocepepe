...
void save_guilds() {
	for (Guild to_save : guilds) {
		...
		ofstream w_("database/guilds/" + to_string(to_save.guild_id) + ".json");
		if (w_.fail()) continue;
		w_ << save_ << endl;
		w_.close();
	}
...
bool guild_access(Player* player) {
	auto guild_id = player->guild_id; 
	if (guild_id == 0 or player->guild_id == 0 or player->guild_id != guild_id) return false;
	vector<Guild>::iterator p = find_if(guilds.begin(), guilds.end(), [guild_id](const Guild& a) { return a.guild_id == guild_id; });
	if (p != guilds.end()) {
		Guild* guild_information = &guilds[p - guilds.begin()];
		uint32_t my_rank = 0;
		for (GuildMember member_search : guild_information->guild_members) {
			if (member_search.member_name == player->tankIDName) {
				my_rank = member_search.role_id;
				break;
			}
		}
							...
							if (pInfo(currentPeer)->guild_id == guild_information->guild_id) {
								if (guild_access(currentPeer, guild_information->guild_id)) {
									gamepacket_t p3(0, pInfo(currentPeer)->netID);
									{
										...
										p3.Insert((not pInfo(currentPeer)->d_name.empty() ? pInfo(currentPeer)->d_name : pInfo(currentPeer)->name_color + pInfo(currentPeer)->tankIDName) + (pInfo(currentPeer)->is_datemaster ? " `4Datemaster" : "") + (pInfo(currentPeer)->is_legend ? " of Legend" : "") + "``");
			...
			if (member_search->member_name == member_name) {
				if (member_search->display_name == "") {
					...
					if (ifs.is_open()) {
						json j;
						ifs >> j;
						if (j["vip"] == 1) member_search->display_name = "`1[VIP] " + member_search->member_name;
						if (j["vvip"] == 1) member_search->display_name = "`9[VVIP] " + member_search->member_name;
						if (j["glory"] == 1) member_search->display_name = "`c[GLORY] " + member_search->member_name;
						if (j["mod"] == 1) member_search->display_name = "`#@" + member_search->member_name;
						if (j["supermod"] == 1) member_search->display_name = "`b@" + member_search->member_name;
						if (j["admin"] == 1) member_search->display_name = "`@" + member_search->member_name;
						if (j["dev"] == 1) member_search->display_name = "`6@" + member_search->member_name;
						if (is_reseller(j["userID"].get<int>()) and j["hide_reseller"].get<bool>() == false) member_search->display_name += " `c[RESELLER]``";
						//if (pInfo(peer)->show_uid) member_search->display_name += format("`4({})``", j["userID"].get<int>());
					}
				}
...
string get_guild_info(const ENetPeer* peer, bool all_ = false) {
	uint32_t guild_id = pInfo(peer)->guild_id;
	if (guild_id == 0) {
		return "set_default_color|`o\nadd_label_with_icon|big|`wGrow Guild ``|left|5814|\nadd_textbox|Welcome to Grow Guilds where you can create a Guild! With a Guild you can compete in Guild Leaderboards to earn rewards and level up the Guild to add more members.|\nadd_spacer|small|\nadd_textbox|To create a Guild you must either be a  `2Supporter`` or `2Super Supporter``.|\nadd_textbox|The cost for `2Super Supporters`` is: `6" + comma(guild_create_gem_requirement / 2) + "`` Gems.|\nadd_spacer|small|\nadd_textbox|You will be charged: `6" + comma((pInfo(peer)->supp == 1 ? guild_create_gem_requirement / 2 : guild_create_gem_requirement)) + "`` Gems.|\nadd_spacer|small|\nadd_textbox|`8Caution``: A guild can only be created in a world owned by you and locked with a `5World Lock``!|\nadd_spacer|small|\nadd_button|showcreateguild|Create a Guild|noflags|0|0|\nadd_button|back|Back|noflags|0|0|\nend_dialog|requestcreateguildpage|Close||";
	}
	uint32_t my_role = 0;
	vector<Guild>::iterator p = find_if(guilds.begin(), guilds.end(), [guild_id](const Guild& a) { return a.guild_id == guild_id; });
	if (p != guilds.end()) {
		...
		for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
			...
			for (GuildMember search_member : guild_information->guild_members) {
				if (search_member.member_name == pInfo(currentPeer)->tankIDName) {
					string display_color = (pInfo(currentPeer)->dev == 1 ? "`6@" : (pInfo(currentPeer)->admin == 1) ? "`8@" : (pInfo(currentPeer)->supermod == 1) ? "`b@" : (pInfo(currentPeer)->tmod == 1) ? "`#@" : pInfo(currentPeer)->glory == 1 ? "`c[GLORY] " : pInfo(currentPeer)->vvip == 1 ? "`9[VVIP]" : pInfo(peer)->vip == 1 ? "`1[VIP] " : "`o");
					ok.push_back(pInfo(currentPeer)->tankIDName);
					online_guild_members.push_back("\nadd_button|" + pInfo(currentPeer)->tankIDName + "|`2ONLINE: ``" + display_color + pInfo(currentPeer)->tankIDName + (is_reseller(currentPeer) and not pInfo(currentPeer)->hide_reseller ? " `c[RESELLER]" : "") + "``" + (search_member.role_id != 0 ? (search_member.role_id == 1 ? " `9(GE)``" : (search_member.role_id == 2 ? " `6(GC)``" : (search_member.role_id == 3 ? " `e(GL)``" : ""))) : "") + "|noflags|0|0|");
					break;
				}
			}
		} if (all_) {
				...
				if (find(ok.begin(), ok.end(), search_member->member_name) == ok.end()) {
					if (search_member->display_name == "") {
						...
						if (ifs.is_open()) {
							json j;
							ifs >> j;
							if (j["vip"] == 1) search_member->display_name = "`1[VIP] " + search_member->member_name;
							if (j["vvip"] == 1) search_member->display_name = "`9[VVIP] " + search_member->member_name;
							if (j["glory"] == 1) search_member->display_name = "`c[GLORY] " + search_member->member_name;
							if (j["mod"] == 1) search_member->display_name = "`#@" + search_member->member_name;
							if (j["supermod"] == 1) search_member->display_name = "`b@" + search_member->member_name;
							if (j["admin"] == 1) search_member->display_name = "`8@" + search_member->member_name;
							if (j["dev"] == 1) search_member->display_name = "`6@" + search_member->member_name;
							if (is_reseller(j["userID"].get<int>()) and j["hide_reseller"].get<bool>() == false) search_member->display_name += " `c[RESELLER]``";
						}
					}
